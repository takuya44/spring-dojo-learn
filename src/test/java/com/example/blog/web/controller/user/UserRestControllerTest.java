...
class UserRestControllerTest {
  ...
  /**
   * POST /users: ユーザー名の長さまたは構成する文字列に違反がある場合、400 Bad Request を返すテスト。
   *
   * <p>このテストでは、以下の条件でエラーが発生することを確認します:</p>
   * <ul>
   *   <li>ユーザー名がバリデーションルール（長さ/文字構成）に違反している。</li>
   ...
   */
  @Test
  @DisplayName("POST /users：ユーザー名の長さ/構成する文字列に違反がある時、400 Bad Request")
  public void createUser_badRequest_invalidUsername() throws Exception {
    // ## Arrange ##
    // バリデーションに違反するユーザー名（先頭にドットがある）を含むリクエストボディを作成
    var newUserJson = """
        {
          "username": ".username",
          "password": "password00"
        ...
        .andExpect(jsonPath("$.errors", hasItem(
                allOf(
                    ...
                    hasEntry("detail", // 詳細なエラーメッセージを確認
                        "ユーザー名は3文字以上32文字以内で入力してください。半角英数字、ハイフン、アンダースコア、ドットのみを使用できます。先頭と末尾にハイフン、アンダースコア、ドットを使用することはできません。")
                )
            )
        ));
  }
